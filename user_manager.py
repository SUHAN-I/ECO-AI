...
# Memorable ID format: WORD-WORD-NNN  e.g. ECO-LEAF-427
_W1 = ["ECO","GREEN","CLEAN","BLUE","PURE","FRESH","SAFE","SMART","BRIGHT","CLEAR"]
_W2 = ["LEAF","CAN","BOTTLE","RIVER","STAR","WAVE","SEED","TREE","STONE","EARTH"]
...
def generate_user_id():
    """Returns a short memorable ID like ECO-LEAF-293."""
    return f"{random.choice(_W1)}-{random.choice(_W2)}-{random.randint(100,999)}"
...
def ensure_sheets(sheet):
    ...
    if USERS_SHEET not in existing:
        ws = sheet.add_worksheet(title=USERS_SHEET, rows=1000, cols=10)
        ws.append_row(["user_id","name","city","registered_at",
...
# ════════════════════════════════════════════════════════════
def register_new_user(db, name, city, language="english"):
    """
    Creates a new user, saves to Sheets, returns user dict.

    Returns:
        user dict with user_id, name, city, etc.
        or None if failed
    """
    sheet = db.get("sheet")
    if not sheet:
    ...
    try:
        ...
        ws.append_row([uid, name, city, now, now, 0, language])

        print(f"✅ Registered: {name} → {uid}")
        return {"user_id":uid,"name":name,"city":city,
